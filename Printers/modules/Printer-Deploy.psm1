...
function Get-IntuneWinInnerFile {
    param([string]$SourceFile, [string]$FileName)
...
function Invoke-PrinterDeploy {
    ...
    $appName      = "Printer - $PrinterName"
    $publisher    = "IT Department"
    $installCmd   = "powershell.exe -executionpolicy bypass -file deploy.ps1"
    $uninstallCmd = "powershell.exe -executionpolicy bypass -file remove.ps1"
    $description  = "Printer deployment: $PrinterName - deployed via IntuneConfigurator"
    ...
    try {
        $existingResp = Invoke-MgGraphRequest -Method GET `
            -Uri "$baseUri/mobileApps?`$filter=displayName eq '$appName'" -ErrorAction Stop
                ...
                foreach ($app in $existingApps) {
                    try {
                        Invoke-MgGraphRequest -Method DELETE `
                            -Uri "$baseUri/mobileApps/$($app.id)" -ErrorAction Stop | Out-Null
    ...
    # Build detection rule from detect.ps1
    Write-Host "Preparing script detection rule..." -ForegroundColor Yellow
    try {
        $detectContent = Get-Content $DetectScriptPath -Raw -Encoding UTF8
        $detectB64     = [Convert]::ToBase64String([System.Text.Encoding]::UTF8.GetBytes($detectContent))
    }
    catch {
        Write-Error "Failed to read detect.ps1: $($_.Exception.Message)"
        return
    }
    Write-Host "  detect.ps1 encoded for Graph API." -ForegroundColor Green
    ...
    try {
        ...
        Write-Error "Failed to create app: $($_.Exception.Message)$(if ($detail) {"`n  $detail"})"
        Remove-Item $innerFilePath -Force -ErrorAction SilentlyContinue
        return
    }
...
Export-ModuleMember -Function Invoke-PrinterDeploy
